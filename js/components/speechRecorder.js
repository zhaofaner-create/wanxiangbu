(function (global, factory) {
  ...
  function createSpeechRecorder(options = {}) {
    ...
    async function start() {
      ...
      try {
        // 曾经短暂尝试过关掉回声消除/降噪/自动增益（以为这样能让"教室里发生的一切"都
        // 原样录下来），但用户反馈课堂场景其实不需要这个：不会有人放视频，老师的声音
        // 本身可能离麦克风不算很近，恰恰需要靠自动增益/降噪把这种"不算近但说得清楚"的
        // 人声稳定地捕捉下来——这正是这三项处理本来要解决的场景。所以这里改回让浏览器
        // 用它自己的默认值（一般都是开启的），不做任何干预；这个设置只影响这里保存音频
        // 用的这路采集，跟旁边 SpeechRecognition 自己内部的语音识别采集是两条独立的
        // 通道，不会影响转录准确度。
